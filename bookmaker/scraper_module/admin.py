...
from .models import ScraperStatus, CombinedScraperStatus, LiveScraperStatus
        ...
        if request.method == "POST":
            ...
            if action == "start":
                if not status.is_running:
                    ...
                    async_task('scraper_module.tasks.monitor_main_list_persistent')
...
@admin.register(LiveScraperStatus)
class LiveScraperStatusAdmin(admin.ModelAdmin):
    list_display = ('is_running', 'last_run', 'monitor_link')

    def monitor_link(self, obj):
        return format_html(
            '<a class="button" href="monitor/" style="background-color: #417690; color: white; padding: 5px 10px; border-radius: 5px; text-decoration: none;">Open Monitor Dashboard</a>')
    ...
    def get_urls(self):
        urls = super().get_urls()
        custom_urls = [
            path('monitor_live/', self.admin_site.admin_view(self.monitor_view), name='scraper_monitor'),
            path('control_live/<str:action>/', self.admin_site.admin_view(self.control_view), name='scraper_control'),
            path('logs_live/', self.admin_site.admin_view(self.get_logs_view), name='scraper_logs'),
        ]
        return custom_urls + urls

    def monitor_view(self, request):
        status, _ = LiveScraperStatus.objects.get_or_create(id=1)
        context = dict(
            self.admin_site.each_context(request),
            status=status,
        )
        ...
        if request.method == "POST":
            ...
            if action == "start":
                if not status.is_running:
                    ...
                    status.save()
                    # Launch background task
                    async_task('scraper_module.tasks.monitor_live_page_persistent')
                    return JsonResponse({'status': 'ok', 'message': 'Started'})
                else:
                    return JsonResponse({'status': 'error', 'message': 'Already running'})
