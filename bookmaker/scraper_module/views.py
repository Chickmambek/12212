...
def get_scraper_stats(request):
    try:
        ...
        today_matches = Match.objects.filter(scraped_at__gte=today_start).count()
        recent_matches = Match.objects.filter(scraped_at__gte=hour_ago).count()
        daily_matches = Match.objects.filter(scraped_at__gte=day_ago).count()

        # Scraping speed
        scraping_speed = f"{recent_matches}/hour"
        avg_speed = f"{daily_matches // 24 if daily_matches > 24 else 1}/hour"

        # Database file size (for SQLite – adjust if using PostgreSQL/MySQL)
        db_path = connection.settings_dict['NAME']
        db_size_bytes = os.path.getsize(db_path) if os.path.exists(db_path) else 0
        db_size = f"{db_size_bytes / (1024*1024):.1f} MB" if db_size_bytes > 0 else "0 KB"

        # Log file statistics
        log_stats = {
            'main_scraper': {'exists': False, 'size_kb': 0, 'modified': None},
            'live_scraper': {'exists': False, 'size_kb': 0, 'modified': None}
        }
        ...
        response_data = {
            ...
            'scrapers': {
                'main': {
                    ...
                    'status': 'running' if main_status.is_running else 'stopped',  # optional
                    'log': log_stats['main_scraper']
                },
                'live': {
                    'is_running': live_status.is_running,
                    'last_run': live_status.last_run.isoformat() if live_status.last_run else None,
                    'status': 'running' if live_status.is_running else 'stopped',  # optional
