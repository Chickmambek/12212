...
def matches_dashboard(request):
    ...
    # Apply ordering: league_order first (ascending), then match_date
    live_matches = live_qs.select_related('home_team', 'away_team') \
        .prefetch_related(prefetch_odds) \
        .order_by('league_order', 'match_date')

    upcoming_matches = upcoming_qs.select_related('home_team', 'away_team') \
        .prefetch_related(prefetch_odds) \
        .order_by('league_order', 'match_date')

    today_matches = today_qs.select_related('home_team', 'away_team') \
        .prefetch_related(prefetch_odds) \
        .order_by('league_order', 'match_date')

    finished_matches = finished_qs.select_related('home_team', 'away_team') \
        .order_by('league_order', '-match_date')[:20]
...
def search_matches(request):
    """API endpoint for searching matches"""
    ...
    ).select_related('home_team', 'away_team')[:20]  # Limit to 20 results
    ...
    results = [{
        ...
        'league': m.league,
        'time': m.match_date.strftime('%Y-%m-%d %H:%M'),
        'status': m.status
    } for m in matches]
...
def place_bet(request, match_id):
    """Handle both regular and quick bets"""
    if request.method != 'POST':
        return JsonResponse({'error': 'Invalid request method'}, status=405)

    # Get match first (needed for both quick and regular bets)
    match = get_object_or_404(Match, id=match_id)

    # Handle quick bet requests (simplified params)
    is_quick_bet = request.headers.get('X-Quick-Bet') == 'true'
    if is_quick_bet:
        data = {
            'bet_type': request.headers.get('X-Bet-Type'),
            'amount': request.user.profile.default_stake,
            'odds': request.headers.get('X-Odds')
        }
    else:
        data = None
    ...
    try:
        if is_quick_bet and data:
            # Quick bet: use data from headers
...
def get_leagues(request):
    """API endpoint to fetch all distinct leagues"""
    leagues = Match.objects.order_by('league').values_list('league', flat=True).distinct()
